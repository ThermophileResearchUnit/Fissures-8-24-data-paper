...
path1 <- "C:/Users/snoell/OneDrive - The University of Waikato/Documents/Backup_seqs/Fissure8_amplicons/archaeal"
...
rbind(FWD.ForwardReads = sapply(FWD.orients, primerHits, fns = fn_f[[1]]), 
      REV.ForwardReads = sapply(REV.orients, primerHits, fns = fn_r[[1]]))
...
out <- filterAndTrim(fn_f, filt_f, rev = fn_r, filt.rev = filt_r, truncLen=c(250, 200), minLen = 150,
...
seqtab2 <- seqtab[,nchar(colnames(seqtab)) %in% 254:291]
...
saveRDS(seqtab2, "seqtab.arch")

# Save tracking table
write.csv(track2, "track_arch.csv")
...
# [1] 0.90 - the frequency of non-chimeric sequences

saveRDS(seqtab.nochim, "seqtab.nochim.arch")
```
...
write.fasta(uniqueSeqs, name.seq, "asvs_no-filt_arch.fasta")
```
...
saveRDS(taxa.silva, "taxa.silva.arch")
...
#seqtab.nochim <- readRDS("seqtab.nochim.arch")
envdata <-  read.csv("sample_data.csv", row.names = 1, header = TRUE)
#taxa.silva <- as.matrix(readRDS("taxa.silva.arch"))
...
ntaxa(physeq)
ntaxa(physeq.fin)
#saveRDS(physeq.fin, "physeq.arch")
...
#write.csv(full_df, "asv_info_arch.csv", row.names = FALSE)
```